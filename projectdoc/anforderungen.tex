...
\begin{table}[ht]
    \begin{tabular}{l p{14cm}}
        ...
        \midrule
        \textbf{Admin}
                                & Ein \emph{Admin} ist für die Bereitstellung von akkuraten \gls{ÖV-Güteklassen}-Daten verantwortlich, welche er den Aktoren \emph{Analyst} und \emph{User} für die weitere Auswertung zur Verfügung stellt.
                                So möchte er die erforderlichen Daten sammeln und verarbeiten. \\
        \textbf{Analyst}
                                & Unter dem Aktor \emph{Analyst} sind Raum-/Verkehrsplaner, Transportgesellschaften und weitere Personen zusammengefasst, welche die \gls{ÖV-Güteklassen}-Daten für analytische Zwecke und als Planungsinstrument verwenden möchten. \\
        \textbf{User}
                                & Ein \emph{User} ist grundsätzlich am aktuellen Stand der \gls{ÖV-Güteklassen} in einer bestimmten Region interessiert, welche er als Basis für weitere Entscheidungen bezieht. \\
        \bottomrule
    \end{tabular}
    \caption{Aktoren}
    \label{table:Aktoren}
...
Der \emph{Admin} startet die Berechnung der \gls{ÖV-Güteklassen} 2018 basierend auf einer Konfiguration (Stichtage und Zeitspannen).
...
Der \emph{Admin} füḧrt das manuelle Beziehen des \gls{Terrainmodell} durch und übergibt diese dem System "`\gls{ÖV-Güteklassen} 2018"'.
Das System bereit die Daten so vor, dass sie in \nameref{Use Cases:UC01} für die "`\gls{ÖV-Güteklassen} 2018"'-Berechnung einbezogen werden können
...
\subsubsection{UC03: Fahrplandaten vorverarbeiten}
\label{Use Cases:UC03}

Aktoren: \emph{Admin}

Der \emph{Admin} nutzt das System "`\gls{ÖV-Güteklassen} 2018"' für das Beziehen der Fahrplandaten über eine externe Schnittstelle.
Das System bereit die Daten so vor, dass sie in \nameref{Use Cases:UC01} für die "`\gls{ÖV-Güteklassen} 2018"'-Berechnung einbezogen werden können.
...
Der \emph{Admin} nutzt das System "`\gls{ÖV-Güteklassen} 2018"' für das Beziehen der \acs{OSM}-Daten über eine externe Schnittstelle.
Das System bereit die Daten so vor, dass sie in \nameref{Use Cases:UC01} für die "`\gls{ÖV-Güteklassen} 2018"'-Berechnung einbezogen werden können.
...
Der \emph{Admin} nutzt das System "`\gls{ÖV-Güteklassen} 2018"' für die Integration der bereits berechneten ÖV-Güteklassen ARE~\cite{berechnung_are}.
...
Der \emph{Analyst} nutzt das System "`\gls{ÖV-Güteklassen} 2018"' für den Vergleich der \gls{ÖV-Güteklassen} ARE und 2018.
Das System präsentiert eine grafische Überlagerung der beiden \gls{ÖV-Güteklassen}.
...
Die Aktoren \emph{Analyst} und \emph{User} nutzt das System "`\gls{ÖV-Güteklassen} 2018"', um die berechneten \gls{ÖV-Güteklassen} 2018 anzuzeigen.
...
Die Aktoren \emph{Analyst} und \emph{User} übergibt dem System "`\gls{ÖV-Güteklassen} 2018"' eine Konfiguration der anzuzeigenden \gls{ÖV-Güteklassen} 2018.
Das System "`\gls{ÖV-Güteklassen} 2018"' liefert auf Basis dieser die zugehörigen, vorgerechneten Daten.

\subsection{Nicht-funktionale Anforderungen}
\label{Anforderungsspezifikation:Nicht-funktionale Anforderungen}

Im Folgenden sind die nicht-funktionalen Anforderungen an das System "`\gls{ÖV-Güteklassen} 2018"' aufgelistet.
...
\begin{table}[H]
    \begin{tabular}{l p{10.6cm}}
        ...
        \textbf{Response}
                                & Berechnete ÖV-Güteklassen 2018 als \gls{GeoJSON}\\  
        \textbf{Response Measure}
