...
Diese werden manuell bezogen, da der Datensatz mit ca. 120GB für ein Download über das Web-\ac{API} zu gross ist.
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.8\linewidth]{projectdoc/img/container-diagram}
    \caption[ht]{}
...
Am Schluss der Berechnung wird das Ergebnis in geografischen Formen abgebildet und als GeoJSON an den \nameref{container:Backend}-Container ausgeliefert.
...
Die Web-App dient als Frontend des Systems ''\ac{ÖV}-Güteklassen 2018''.
Die Applikation wird als \ac{SPA} direkt an den Browser des Users ausgeliefert.
Von dort werden dann per JavaScript die Daten im GeoJSON-Format vom \ac{API} des \nameref{container:Backend}-Containers abgerufen und visualisiert.

Dafür wird, wie in den Rahmenbedingungen festgehalten, entweder \emph{Vue.js}~\cite{vuejs} oder \emph{React}~\cite{react} verwendet.
...
Dazu werden zuerst periodisch die Kartendaten von \nameref{subsystem:planet.osm.ch} eingelesen und zusammen mit dem Höhenmodell von \nameref{subsystem:swissALTI3D} eine Routing-Topologie erstellt.
Sobald die Berechnung der ÖV-Güteklassen beginnt, wird die Routing-Engine vom \nameref{container:processor} nach Isolinien abgefragt, die mit Haltestellen als Startpunkt und einer gewissen Distanz (gewichtet nach Höhenunterschied) berechnet werden.
