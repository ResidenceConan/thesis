...
Viel effizienter ist es, die Abfahrtszeiten für alle \glspl{Haltestelle} in einer einzelnen Abfrage zu aggregieren.
...
Ein erster Ansatz dieses "`Snappings"' sucht mithilfe einer "`\gls{Nearest Neighbor Suche}"' den geographisch nächstliegenden \gls{Vertex} des \glspl{Routing-Graph} zu den Koordinaten der \gls{Haltestelle}.
...
Dadurch kann die Web-App entscheiden, welche \gls{ÖV-Güteklassen} der Benutzer auswählen kann und wie diese vom Container \nameref{container:Tile-Server} bezogen werden können.
...
Der Container \nameref{container:generator} generiert die \gls{ÖV-Güteklassen} aktuell im \gls{GeoJSON}-Format.
...
Die Indirektion über das \gls{GeoJSON}-Format ist bewusst.
