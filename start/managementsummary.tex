...
So ist nicht klar, in welchem Mass eine anspruchsvolle Topographie zu Buche schlägt, denn eine \gls{Haltestelle} in einem steilen Gebiet hat ein geringeres Einzugsgebiet als eine \glspl{Haltestelle} in einem flachen Gelände.
...
Dies ist mit freiverfügbaren Kartedaten von \gls{OpenStreetMap} (OSM) und dem hoch aufgelöstem digitalen \gls{Terrainmodell} (DTM) swissALTI$^{3D}$ von Swisstopo möglich.
...
Die Topographie wird durch die Verwendung von \gls{Leistungskilometer}n in der Berechnung des Einzugsgebiet miteinbezogen.
...
\paragraph{Ausblick}~\\
In der vorliegenden Arbeit wurde beim Berechnen der Erreichbarkeit der Fokus auf eine routen-basierte Methode, namentlich \glspl{Isochrone}, gelegt.
...
In Abbildung \ref{fig:mgmt_summary_grid_based_approach} wurde dieser Ansatz für die Berechnung der Reisezeit zu einer \gls{Haltestelle} eines Fussgängers verwendet.
