...
"`\gls{ÖV-Güteklassen} geben Auskunft darüber, wie gut ein Standort mit dem öffentlichen Verkehr erschlossen ist.
...
\paragraph{OeVGK93}~\\
% same description is used in the corresponding glossary entry
\gls{OeVGK93} steht kurz für \gls{ÖV-Güteklassen} 93 und bezeichnet die Definition der \gls{ÖV-Güteklassen}, welche im Jahre 1993 mit der \acs{SN} 640 290~\cite{sn640290} verabschiedet wurde.

\paragraph{OeVGKARE}~\\
% same description is used in the corresponding glossary entry
\gls{OeVGKARE} steht kurz für \gls{ÖV-Güteklassen} des \acl{ARE} und bezeichnet die Spezifikation und Umsetzung der \gls{ÖV-Güteklassen} basierend auf OeVGK93 mit Erweiterungen.

\paragraph{OeVGK18}~\\
% same description is used in the corresponding glossary entry
OeVGK steht kurz für \gls{ÖV-Güteklassen} 2018 und bezeichnet die neue Spezifikation der \gls{ÖV-Güteklassen}, welche im Zuge dieser Arbeit erarbeitet wird.

\subsection{Problemstellung}
\label{Einführung:Problemstellung}

Mit \gls{ÖV-Güteklassen} wird ermittelt, welche Gebiete wie gut mit öffentlichen Verkehrsmitteln erschlossen sind.
...
Die Methodik zur Erhebung von \gls{ÖV-Güteklassen} wurde 1993 erstmals in der \ac{SN} 640 290~\cite{sn640290} festgelegt.
...
Darin sind \gls{ÖV-Güteklassen} in dieser Form nicht mehr erhalten. Die alte Norm wird aber immer noch als Grundlage verwendet, um die aktuellen \gls{ÖV-Güteklassen} zu berechnen~\cite{berechnung_are}.
...
Bei der Berechnung von \gls{ÖV-Güteklassen} nach der \acs{SN} 640 290~\cite{sn640290} werden um jede \gls{Haltestelle} Kreise mit einem gewissen Radius gezogen.
Diese bestimmen die Einzugsgebiete der \glspl{Haltestelle} mit unterschiedlicher Qualität.
Diese Methode beachtet aber nicht die effektive Erreichbarkeit für Fussgänger.
Es kann sein, dass die \gls{Haltestelle} nur durch eine Strasse zugänglich ist und Fussgänger in der Umgebung einen Umweg laufen müssen.
...
In der jetzigen Berechnung der \gls{ÖV-Güteklassen} wird die Topographie der Umgebung von \glspl{Haltestelle} nicht konsequent einberechnet und nur wage definiert, wie weit die Berücksichtigung dieser gehen sollte.
Je steiler der Zugang zu einer \gls{Haltestelle} ist, desto kleiner wird das effektive Einzugsgebiet der \gls{Haltestelle}, da Fussgänger weniger bereit sind, Distanzen zu laufen, die auf ebenem Grund akzeptabel sind.

\subsubsection{Taktberechnung}
\label{Problemstellung:Taktberechnung}

Nach der verwendeten \acs{SN} 640 290~\cite{sn640290} werden \glspl{Haltestelle} anhand der Anzahl der Verbindungen in einer bestimmten Zeit in Kategorien eingeteilt.
Je höher diese Kategorie, desto grösser wird das Einzugsgebiet der \gls{Haltestelle} bewertet.

Bei der Einteilung in die Kategorie wird aber nicht darauf geachtet, in welchem Takt die Verbindungen eintreffen.
So kann es sein, dass an einer \gls{Haltestelle} alle 15 Minuten genau ein Bus ankommt und bei einer anderen alle 30 Minuten gleich zwei Busse in gleicher Richtung hintereinander.
Die Berechnung zählt aber nur die Anzahl Verbindungen insgesamt, womit beide \glspl{Haltestelle} danach gleichwertig wären, obwohl die \gls{Haltestelle} mit dem 15 Minuten Takt offensichtlich einen besseren Takt hat.
...
Für ein Benutzer der \gls{Haltestelle} ist dies allerdings ein markanter Unterschied.

\subsubsection{Berechnung an einem Stichtag}
\label{Problemstellung:Berechnung an einem Stichtag}

Bei der Berechnung von \gls{ÖV-Güteklassen} werden Verbindungen in einem gewissen Zeitraum berücksichtigt.
Dabei werden nur Verbindungen zwischen 6.00 Uhr und 20.00 Uhr gezählt.
Die Ansprüche an die \gls{ÖV-Güteklassen} können aber auch andere Zeiten umfassen.
...
Das \acl{ARE} hat 2011 die Berechnungsmethodik \acs{ARE}~\cite{berechnung_are} herausgegeben, welche die \gls{ÖV-Güteklassen} in einem automatisierten Prozess aus den Daten des elektronischen Fahrplans HAFAS~\cite{sbb_hafas_spec} berechnet.
...
Es existieren unterschiedliche Berechnungsmethodiken, welche \gls{ÖV-Güteklassen} entweder auf nationaler oder kantonaler Ebene berechnen.
...
Statt um jede \acs{ÖV}-\gls{Haltestelle} Umkreise mit fixem Radius zu ziehen, bietet es sich an, das effektive Einzugsgebiet zu analysieren, das ein Fussgänger durch Benutzung von Strassen und Gehwege von der \gls{Haltestelle} aus erreichen kann.

\subparagraph{Topographie}~\\
\label{Evaluierung grundlegender Verbesserungen:Topographie}
Um die Topographie bei der Berechnung vom Einzugsgebiet einer \gls{Haltestelle} mit einzubeziehen, müssen die Höhenunterschiede beachtet werden.
...
Eine \gls{Haltestelle} in einem steilen Gebiet hat so ein geringeres Einzugsgebiet als eine gleich gut angeschlossene \gls{Haltestelle} in einem flachen Gebiet.
...
Wie in \ref{Problemstellung:Taktberechnung} beschrieben, ergibt dies kein genaues Bild der Taktfrequenz der Verbindungen einer \gls{Haltestelle}.
...
Da die Fahrplandaten für das ganze Jahr zur Verfügung stehen und die Berechnung automatisiert erfolgen kann, spricht nichts dagegen, die \gls{ÖV-Güteklassen} für mehrere Stichtage und Zeitintervalle zu ermitteln.
...
Mit einem Generator werden die \gls{ÖV-Güteklassen} basierend auf \gls{OeVGK18} für mehrere Parameter berechnet.
...
Darin können neben unseren Berechnungen auch die bisherigen \gls{ÖV-Güteklassen} (Berechnungsmethodik \acs{ARE}) angezeigt werden.
...
Es wird das Fazit gezogen, dass mit dem kategorialen Ansatz, zu welchem die \gls{ÖV-Güteklassen} gehören, sich inhaltliche Ansprüche am besten mit den Ansprüchen der Nachvollziehbarkeit in der Planungspraxis und Kommunizierbarkeit kombinieren lassen.
...
Diese Bachelorarbeit befasst sich mit dem Problem, eine den heuten technischen Möglichkeiten entsprechende \gls{ÖV-Güteklassen}-Spezifikation (\gls{OeVGK18}) und deren Berechnung umzusetzen.
...
So verfolgt die erste Phase das Ziel, eine \gls{ÖV-Güteklassen} 2018 Spezifikation zu erstellen, mit dem Fokus, schweizweit Akzeptanz zu erhalten.
...
In einem zweiten Teil wird die erarbeitete Spezifikation implementiert und die \gls{ÖV-Güteklassen} für die gesamte Schweiz berechnet.
Die Visualisierung erfolgt in Form einer Web-Applikation, worin die \gls{ÖV-Güteklassen} auf einer interaktiven Karte dargestellt sind.
